...
class DetailViewController: UIViewController
{
    ...
    override func viewDidLoad()
    {
        ...
        viewModelDetails.getDetails(id: id, completion: { [weak self] in
            self?.mealNameLabel.text = self?.viewModelDetails.details[0].strMeal
           
            guard let imageURL = URL(string: (self?.viewModelDetails.details[0].strMealThumb)!) else { return }
            ...
            self?.measure1Label.text = self?.viewModelDetails.details[0].strMeasure1
            self?.measure2Label.text = self?.viewModelDetails.details[0].strMeasure2
            self?.measure3Label.text = self?.viewModelDetails.details[0].strMeasure3
            self?.ingredient1Label.text = self?.viewModelDetails.details[0].strIngredient1
            self?.ingredient2Label.text = self?.viewModelDetails.details[0].strIngredient2
            self?.ingredient3Label.text = self?.viewModelDetails.details[0].strIngredient3
            self?.mealDescription.text = self?.viewModelDetails.details[0].strInstructions
            // tagurile
            let array = self?.viewModelDetails.details[0].strTags
